let videoclub = {
  peliculas: [
    ...
    },
    {
      ...
      sinopsis:
        "En la ciudad comienzan a aparecer mujeres muertas sin una gota de sangre en sus cuerpos y con extrañas marcas de colmillos. La curia del lugar sospecha que un súcubo de los avernos es responsable, así que convocan el auxilio de Nuestro Señor Jesucristo",
      cartel:
    ...
    },
    {
      ...
      sinopsis:
        "Una planta nuclear vierte unos desechos tóxicos en la ciudad de Nueva Jersey, cerca de una escuela provocando que dos estudiantes se conviertan en monstruos mutantes.",
    ...
    },
    {
      ...
      sinopsis:
        "Un científico descubre en Skull Island un ejemplar muy extraño de mono rata al que acompaña una terrible maldición, según los nativos. El ejemplar es trasladado a Nueva Zelanda para su estudio. Cuando Lionel y Paquita realizan una visita al zoológico, la madre de Lionel les vigila en secreto, y será mordida por el animal encontrado en la isla, quien se va convirtiendo en una especie de zombie, sedienta de carne, y que convierte en zombie a todo el que ataca.",
      cartel:
    ...
    },
    {
      ...
      sinopsis:
        "Una triste y gris dependienta, gran admiradora de Elvis y con un pretendiente, verá su vida cambiar cuando en su vida se cruce un apuesto rocker.",
      cartel:
    ...
    },
    {
      ...
      sinopsis:
        "Un grupo de niños han desaparecido misteriosamente. Un escritor y un policía comienzan la búsqueda de estos niños, encontrándose con la sorpresa de que estos infantes se han convertido en unos caníbales asesinos.",
      cartel:
...
for (let i = 0; i < videoclub.peliculas.length; i++) {
  peli += `
    <div class="card" id="pelicula ${videoclub.peliculas[i].titulo}">
        <img src="${videoclub.peliculas[i].cartel}" alt="cartel de la película ${videoclub.peliculas[i].titulo}">
        <div class="container">
        <h2>${videoclub.peliculas[i].titulo}</h2>
        <p><b>Sinopsis:</b> ${videoclub.peliculas[i].sinopsis}</p>
            </div>
        </div>
        `;
}
document.getElementById("div").innerHTML = peli;
